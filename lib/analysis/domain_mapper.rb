...
module Analysis
  class DomainMapper
    ...
    DEFAULT_DOMAIN_RULES_FILE = File.expand_path('../../config/domain_rules.yml', __dir__)
      ...
      list_folders.each do |folder|
        folder[:domains].each do |domain|
          ...
          related_domains.each do |related_domain|
            @mappings[related_domain] = folder[:name] unless has_folder_for_domain?(related_domain)
          end
        end
        ...
        suggested_domains.each do |domain|
          @mappings[domain] = folder[:name] unless @mappings.key?(domain) || has_folder_for_domain?(domain)
        end
      end
    ...
    def find_related_domains(domain)
      ...
      domain_mappings.each do |pattern, related_domains|
        return related_domains if domain_lower.match?(pattern)
      end
    ...
    def suggest_domains_for_folder(folder_name)
      ...
      folder_suggestions.each do |pattern, suggested_domains|
        return suggested_domains if folder_lower.match?(pattern)
      end
