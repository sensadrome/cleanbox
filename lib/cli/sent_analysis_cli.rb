...
module CLI
  class SentAnalysisCLI
    ...
    def compare_sent_with_folders
      ...
      return unless low_overlap_folders.any?
    ...
    def save_csv_data(data)
      ...
      CSV.open(csv_path, 'w') do |csv|
        csv << %w[recipient message_id date]
      ...
      CSV.open(csv_path, 'w') do |csv|
        csv << %w[folder categorization sender message_id date]
      ...
      CSV.open(csv_path, 'w') do |csv|
        csv << %w[folder categorization sender in_sent sent_count]
