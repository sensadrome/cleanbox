...
module CLI
  class SetupWizard
    ...
    def generate_domain_mappings
      ...
      # Get all list folders that were categorized
      list_folders = @analysis_results[:folders].select { |f| f[:categorization] == :list }
      
      # For each list folder, find related domains that might send to it
      list_folders.each do |folder|
        folder_domains = folder[:domains]
        
        # Find related domains for this folder's domains
        folder_domains.each do |domain|
          related_domains = find_related_domains(domain)
          
          related_domains.each do |related_domain|
            # Only map if we don't already have a folder for this domain
        ...
        # Also suggest mappings based on folder name (for initial setup)
        # This helps when users haven't moved emails to folders yet
        suggested_domains = suggest_domains_for_folder(folder[:name])
        suggested_domains.each do |domain|
          # Only suggest if we don't already have this domain mapped
    ...
    def suggest_domains_for_folder(folder_name)
      # Suggest common domains based on folder name for initial setup
      # This is a fallback when no domains are found in the folder yet
    ...
    def find_related_domains(domain)
      # Find variations of known domains based on the actual domain found
      case domain.downcase
      when /github\.com/
        ['githubusercontent.com', 'github.io', 'githubapp.com']
      when /facebook\.com/
    ...
    def has_folder_for_domain?(domain)
      # Check if any existing folder already handles this domain
      @analysis_results[:folders].any? do |folder|
